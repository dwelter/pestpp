...
# endif
  ...
  */
  ...
  int ncol = solver1.getNumCols();
  int nrow = solver1.getNumRows();
  const double * sol_dec = solver1.getColSolution();
  const double * sol_const = solver1.getRowActivity();
  const std::vector<std::string> columnNames = solver1.getColNames();
  const std::vector<std::string> rowNames = solver1.getRowNames();
  for (int i = 0; i < ncol; i++)
	  std::cout << columnNames[i] << ": " << sol_dec[i] << std::endl;
  for (int i = 0; i < nrow; i++)
	  std::cout << rowNames[i] << ": " <<sol_const[i] << std::endl;
