...
ncol = 30
nper = 12
spdict = {"sp":[0,4,8],"sm":[1,5,9],"fl":[2,6,10],"wn":[3,7,11]}

def build_wel_file():
    ml = flopy.modflow.Modflow.load("supply2.nam", check=False, verbose=True)
    df = pd.DataFrame.from_records(ml.sfr.reach_data)
    print(df.columns)
    reach_tups = df.apply(lambda x:(x.i,x.j),axis=1)
    wel_ij = []
    ib = ml.bas6.ibound.array
    for i in range(ml.nrow):
        for j in range(ml.ncol):
            if ib[0,i,j] > 0 and (i,j) not in reach_tups:
                wel_ij.append((i,j))
    wel_data = {}
    for iper in range(nper):
        prefix = None
        wd = []
        for pr,sps in spdict.items():
            if iper in sps:
                prefix = pr
                break
        for (i,j) in wel_ij:
            name = "i{0:02d}j{1:02d}_{2}".format(i,j,prefix)
            wd.append([1,i+1,j+1,0.0,name])
        wel_data[iper] = wd
...
def write_wel_tpl():
    f_in = open("supply2.wel",'r')
    f_tpl = open("supply2.wel.tpl",'w')
    f_tpl.write("ptf ~\n")
    for line in f_in:
        #print(line.strip()[-2:])
        if line.strip()[-2:] in spdict.keys():
            #print(line)
            line = line.replace("0.0000000E+00","~ {0} ~".format(line.strip().split()[-1]))
        f_tpl.write(line)
...
def build_control_file():
    ...
    #par.loc[im_names,"parchglim"] = "factor"
    ...
    par.loc[t_names,"parchglim"] = "factor"
    ...
    par.loc[seg_names,"parchglim"] = "factor"
    ...
    pst.parameter_groups.loc["external","inctyp"] = "absolute"
    pst.parameter_groups.loc["external","derinc"] = 5000.0
    ...
    pst.model_command = ["python supply2.py"]
    #obj_eq = ' 1.095 * q1 + 0.276 * q2a + 0.273 * q2b + 0.273 * q2c + 0.273 * q2d + 0.273 * q4a + 0.273 * q4b'
    obj_eq = ''
    par = pst.parameter_data
    pnames = par.loc[par.pargp.apply(lambda x: x in ["pumping","external"]),"parnme"]

    for pname in pnames:
        obj_eq += ' 0.274 * {0} +'.format(pname)
    ...
    for pname in pnames:
        dec_var_pi_str += ' 1.0 * {0} +'.format(pname)
    dec_var_pi_str = dec_var_pi_str[:-1]
    yr_mn = {0:30000.0,1:25000,2:45000.0,3:0.0}
    ext_str = " 1.0 * im9 + 1.0 * im10 + 1.0 * im11 + 1.0 * im12 +"
    for yr in range(4):
        pi_eq = dec_var_pi_str + ' = {0}'.format(80000)
        if yr == 3:
            pi_eq = ext_str + pi_eq
        pi_eqs.append(pi_eq)
        pi_groups.append("less_pi")
        pi_eq = dec_var_pi_str + ' = {0}'.format(yr_mn[yr])
        if yr == 3:
            pi_eq = ext_str + pi_eq
        pi_eqs.append(pi_eq)
