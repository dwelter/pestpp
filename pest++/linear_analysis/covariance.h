...
public:
	...
	Mat get_matrix(const vector<string> &row_names, const vector<string> &col_names);
	...
	void align_rows(vector<string> &other_row_names);
	void align_cols(vector<string> &other_col_names);
...
public:	
	...
	void from_uncertainty_file(string &filename);
	void from_parameter_bounds(Pest &pest_scenario);
	void from_observation_weights(Pest &pest_scenario);

	void from_parameter_bounds(string &pst_filename);
	void from_observation_weights(string &pst_filename);

	void to_uncertainty_file(string &filename);
private:
	Pest pest_scenario;
