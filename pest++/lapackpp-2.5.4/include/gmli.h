...
public:
     ...
     */
    LaGenMatLongInt(value_type* v, int m, int n, bool row_ordering = false);
    ...
    inline value_type* addr() const;       // begining addr of data space
    //@}
     ...
     * Optional runtime bounds checking (0<=i<m, 0<=j<n) is set
     * by the compile time macro LA_BOUNDS_CHECK. */
    inline const value_type& operator()(int i, int j) const;
     ...
     * affected. */
    LaGenMatLongInt& operator=(value_type s);
     ...
     */
    LaGenMatLongInt& operator=(const LaGenMatLongInt& s); //copy
    ...
    LaGenMatLongInt& add(value_type s);
    ...
    {
        *(const_cast<LaGenMatLongInt*>(this)->info_) = 1;
        return *this;
    };
     ...
     * are printed. */
    friend DLLIMPORT std::ostream& operator<<(std::ostream&, const LaGenMatLongInt&);
...
namespace la
{
/** The matrix data type containing \c long \c int values. */
...
inline LaGenMatLongInt::value_type* LaGenMatLongInt::addr() const
{
    return  v.addr();
}
...
inline LaGenMatLongInt::value_type& LaGenMatLongInt::operator()(int i, int j)
{
...
inline const LaGenMatLongInt::value_type& LaGenMatLongInt::operator()(int i, int j) const
{
