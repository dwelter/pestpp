...
DLLIMPORT int LaGenMatLongInt::debug_ = 0;     // turn off global deubg flag initially.
// use A.debug(1) to turn on/off,
// and A.debug() to check current status.

DLLIMPORT int* LaGenMatLongInt::info_ = new int;       // turn off info print flag.
...
LaGenMatLongInt::LaGenMatLongInt(int m, int n)
    : v(m*n)
{
    init(m, n);
}
...
LaGenMatLongInt::LaGenMatLongInt(value_type *d, int m, int n, bool row_ordering)
    : v(d, m, n, row_ordering)
{
    init(m, n);
    if (debug())
    {
        std::cout << ">>> LaGenMatLongInt::LaGenMatLongInt(double *d, int m, int n)\n";
    }
}

LaGenMatLongInt::LaGenMatLongInt(const LaGenMatLongInt& X)
    : v(0)
{
    debug_ = X.debug_;
    ...
    if (debug())
    {
        ...
        std::cout << "<<< LaGenMatLongInt::LaGenMatLongInt(const LaGenMatLongInt& X)\n";
    }
}

void LaGenMatLongInt::init(int m, int n)
{
    if (m && n)
    {
        ii[0](0, m - 1);
...
LaGenMatLongInt LaGenMatLongInt::operator()(const LaIndex& II, const LaIndex& JJ) const
{
    if (debug())
    {
        std::cout << ">>> LaGenMatLongInt::operator(const LaIndex& const LaIndex&)\n";
    }
    LaIndex I, J;
