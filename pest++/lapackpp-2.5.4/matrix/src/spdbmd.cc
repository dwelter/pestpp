...
DLLIMPORT double LaSpdBandMatDouble::outofbounds_ = 0; // initialize outofbounds_.

DLLIMPORT int LaSpdBandMatDouble::debug_ = 0; // initialize debug to off.

DLLIMPORT int* LaSpdBandMatDouble::info_ = new int; // turn off info print flag.
...
double& LaSpdBandMatDouble::operator()(int i, int j)
{
    ...
    {
        std::cerr << "Index to SPD Banded Matrix out of range!\n";
        assert(0);
        exit (1);
    }
#endif
