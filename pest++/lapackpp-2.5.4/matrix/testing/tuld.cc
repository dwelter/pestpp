...
LaUnitLowerTriangMatDouble madd(const LaUnitLowerTriangMatDouble &A, const LaUnitLowerTriangMatDouble &B)
{
    if (A.debug())
    {
        std::cout << ">>> madd(A,B) \n";
    ...
    {
        std::cerr << "LaUnitLowerTriangMatDouble madd(LaUnitLowerTriangMatDouble &A, LaUnitLowerTriangMatDouble &B): \
            non-conformant arrays.\n";

        return LaUnitLowerTriangMatDouble(0, 0);    // 0x0 matrix
    }

    int i, j;
    LaUnitLowerTriangMatDouble result(A.size(0), A.size(1));
...
int main(int argc, char *argv[])
{
    ...
    LaUnitLowerTriangMatDouble A;
    std::cout << std::endl << "null consturctor " << std::endl;
    std::cout << "A(): " << A.info() << std::endl;

    LaUnitLowerTriangMatDouble C(M, N);
