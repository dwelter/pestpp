...
LaRowVectorDouble madd(const LaRowVectorDouble &A, const LaRowVectorDouble &B)
{
    if (A.debug())
    {
        std::cout << ">>> madd(A,B) \n";
    ...
    if (A.size() != B.size())
    {
        std::cerr << "LaRowVectorDouble madd(LaRowVectorDouble &A, LaRowVectorDouble &B): \
            non-conformant arrays.\n";

        return LaRowVectorDouble(0);       // 0x0 matrix
    }

    int i;
    LaRowVectorDouble result(A.size());
...
int main(int argc, char *argv[])
{
    ...
    LaRowVectorDouble D(C);        // D is also N,N
